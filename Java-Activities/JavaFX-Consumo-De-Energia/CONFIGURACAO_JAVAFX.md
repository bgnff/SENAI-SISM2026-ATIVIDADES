...
Edite o arquivo `compilar.bat` e substitua `CAMINHO_DO_JAVAFX` pelo caminho onde você extraiu o JavaFX:
...
mkdir bin
javac -d bin -cp "%JAVAFX_PATH%\*" --module-path "%JAVAFX_PATH%" --add-modules javafx.controls,javafx.fxml src/application/Main.java src/model/Equipamento.java src/controller/*.java
if %errorlevel% equ 0 (
...
Se você puder usar Maven, a configuração seria mais simples. Adicione ao pom.xml:
