...
shippers.ShipperID AS 'ID DA TRANSPORTADORA'
FROM orders
...
SELECT products.SupplierID AS 'ID DO FORNECEDOR',
products.ProductName AS 'NOME DO PRODUTO',
suppliers.SupplierName AS 'NOME DO FORNECEDOR',
suppliers.Country AS 'PAÍS'
FROM products
INNER JOIN suppliers ON suppliers.SupplierID = products.SupplierID
WHERE suppliers.Country IN ('Brazil','USA');
...
SELECT suppliers.SupplierName AS 'NOME DO FORNECEDOR',
products.ProductName AS 'NOME DO PRODUTO',
products.Price AS 'PREÇO'
FROM suppliers
...
-- 14. Liste o nome do produto, o nome do fornecedor e o nome da transportadora para um pedido específico (ex: OrderID 10248).
...
SELECT suppliers.SupplierName AS 'NOME DO FORNECEDOR',
products.ProductName AS 'NOME DO PRODUTO',
orderdetails.OrderID AS 'ID DO PEDIDO'
FROM suppliers
JOIN products ON  products.SupplierID = suppliers.SupplierID
...
SELECT Suppliers.ContactName AS 'NOME DO CONTATO DO FORNECEDOR',
Suppliers.Country AS 'PAÍS DO FORNECEDOR',
Products.ProductName AS 'NOME DO PRODUTO',
Categories.CategoryID AS 'CATEGORIA'
FROM Suppliers
JOIN Products ON Products.SupplierID = Suppliers.SupplierID
JOIN Categories ON Categories.CategoryID = Products.CategoryID
WHERE Categories.CategoryID IN (1,3,5) AND Suppliers.Country = 'UK';
...
SELECT Suppliers.SupplierName AS 'NOME DO FORNECEDOR',
Suppliers.SupplierID AS 'ID DO FORNECEDOR',
COUNT(OrderDetails.OrderID) AS 'QUANTIDADE TOTAL DE PEDIDOS'
FROM Products
JOIN Suppliers ON Suppliers.SupplierID = Products.SupplierID
JOIN OrderDetails ON OrderDetails.ProductID = Products.ProductID
GROUP BY Suppliers.SupplierName, Suppliers.SupplierID;

-- 19. Mostre o nome do produto, o nome da categoria e o nome do fornecedor, onde o preço do produto é superior à média de preços de todos os produtos.
